...
bool CallAnalyzer::analyzeCall(CallSite CS) {
  ...
  Threshold += VectorBonus;
	...
	{
		LiveAnalyzer LA;
		//errs() << "Entered Live Analyzer\n" ;
//		errs() << "Threshold " << Threshold << "\tCost " << Cost << "\n";
		double ratio1 = (double)Threshold/(double)Cost;
		double ratio2 = (double)Cost/(double)Threshold;	
//		double mu = 25 * ratio2;
//		double beta = 25 * ratio1;	
//		errs() << "ratios " << ratio1 << "\t" << ratio2 << "\n";
		double RegSpil = LA.analyzeCallSite(CS);
//		errs() << RegSpil << " RegSpil\n";
		double Pressure = exp(-1*exp(-1*((ratio2-(RegSpil-1))/(ratio1))));
		Threshold -= Threshold * (Pressure * (RegSpil));
//		errs() << "Threshold " << Threshold << "\t" << (Pressure * (RegSpil)) << "\n";
	}
